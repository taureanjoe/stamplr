...
import { MI_SVG } from "./mi-svg-data";
...
export function renderMichiganStamp(options: StampRenderOptions): string {
  let svg = MI_SVG;
  if (!svg) return fallbackMIStamp(options);
...
function fallbackMIStamp(options: StampRenderOptions): string {
  const name = (options.name ?? "").trim().toUpperCase();
  const license = (options.licenseNumber ?? "").trim();
